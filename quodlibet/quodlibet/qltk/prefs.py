...
from quodlibet.config import RATINGS, ENERGY, DurationFormat, DURATION
...
class PreferencesWindow(UniqueWindow):
    ...
    class Browsers(Gtk.VBox):
        ...
        def __init__(self):
            ...
            # Energy
            cb = CCB(_("Confirm multiple _energy ratings"),
                     'browsers', 'energy_confirm_multiple', populate=True,
                     tooltip=_("Ask for confirmation before changing "
                               "the energy of multiple songs at once"))

            vbox = Gtk.VBox(spacing=6)
            vbox.pack_start(self.energy_vbox(), False, True, 0)
            vbox.pack_start(cb, False, True, 0)
            f = qltk.Frame(_("Energy"), child=vbox)
            self.pack_start(f, False, True, 0)
